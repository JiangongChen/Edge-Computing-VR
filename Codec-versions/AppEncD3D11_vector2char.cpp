...
extern "C"
{
    EXPORT_API int _stdcall encD3D(void* rTex)
    //int main()
    {
    ...
    EXPORT_API int _stdcall Encode(void* rTex)
    {
        if (nFrame > FrameNum) {
            nFrame++;
            char* pkg;
        ...
        for (std::vector<uint8_t>& packet : vPacket)
        {
            /*
            if (i == 0) {
                strcpy_s(pkg, pkg_size, reinterpret_cast<char*>(packet.data()));
            }
            else {
                strcat_s(pkg, pkg_size, reinterpret_cast<char*>(packet.data()));
            }
            i++;*/
            
            for (int i = 0; i < packet.size(); i++)
                pkg.push_back(packet[i]);
            //fpOut->write(reinterpret_cast<char*>(pkg.data()), packet.size());
            //pkg = reinterpret_cast<char*>(packet.data());
            //fpOut->write(pkg, packet.size());
        }
