...
void ParseCommandLine(char *szInputFileName, int &nWidth, int &nHeight, 
...
{
    std::ostringstream oss;
    sprintf(szInputFileName, "%s", "D:/UnityDemo/NativePlugin/build_result/input.yuv");
    sprintf(szOutputFileName, "%s", "D:/UnityDemo/NativePlugin/build_result/output.mp4");
    sscanf("640x360", "%dx%d", &nWidth, &nHeight);
    /*
    int i;
    for (i = 1; i < argc; i++)
    {
...
extern "C"
{
    EXPORT_API const char* EncodeImg()
    {
        ...
        try
        {
            ...
            //ParseCommandLine(szInFilePath, nWidth, nHeight, eFormat, szOutFilePath, encodeCLIOptions, iGpu, 
             //                bOutputInVideoMem, cuStreamType);
            sprintf(szInFilePath, "%s", "D:/UnityDemo/NativePlugin/build_result/input.yuv");
            sprintf(szOutFilePath, "%s", "D:/UnityDemo/NativePlugin/build_result/output.mp4");
            sscanf("640x360", "%dx%d", &nWidth, &nHeight);
        ...
        return "Encoding is initialized.";
    }

    EXPORT_API const char* EncodeCuda()
    {
        // Open input file
...
int main()
{
    ...
    try
    {
        ...
        msg << "Bitstream saved in file " << szOutFilePath << std::endl;
