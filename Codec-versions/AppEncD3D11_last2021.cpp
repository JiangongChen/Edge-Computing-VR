...
extern "C"
{
    // get the resouces to create texture
    EXPORT_API void _stdcall GetResources(void* tex, void** DevicePtr, void** ContextPtr)
    {
        ...
        *DevicePtr = (void*)pDevice.Get();
        *ContextPtr = (void*)pContext.Get();

        //NvEncoderD3D11* enc = (NvEncoderD3D11*)encPtr;
        //ID3D11Device* pDevice = (ID3D11Device*)enc->GetDevice();
    }

    // create the texture to be used for the copy
    EXPORT_API void* _stdcall CreateTex(void** DevicePtr, void* tex, int width, int height)
    {
        ID3D11Device* pDevice = (ID3D11Device*)*DevicePtr;

        ID3D11Texture2D* targetTextures = NULL;
        ID3D11Texture2D* d3dTex = (ID3D11Texture2D*)tex;
        D3D11_TEXTURE2D_DESC desc;
        //get the properties of the texture in Unity
        d3dTex->GetDesc(&desc);
        ...
        ck(pDevice->CreateTexture2D(&desc, NULL, &targetTextures));

        //pContext->Release();

        return (void*)targetTextures;
    }

    // copy the texture from renderTexture to support pipelining
    EXPORT_API void _stdcall CopyTex(void* tex, void* targetTex, void** ContextPtr)
    {
        ID3D11Texture2D* d3dTex = (ID3D11Texture2D*)tex;
        ID3D11Texture2D* targetTextures = (ID3D11Texture2D*)targetTex;
        //ComPtr<ID3D11Device> pDevice;
        //d3dTex->GetDevice(&pDevice);
        ...
        //pDevice->GetImmediateContext(&pContext);
        //ID3D11Device* pDevice = (ID3D11Device*)*DevicePtr;
        ID3D11DeviceContext* pContext = (ID3D11DeviceContext*)*ContextPtr;

        pContext->CopyResource(targetTextures, d3dTex); //asynchronous call 
        pContext->Flush(); // copy the resource immediately
        //pContext->Release();
    }

    // release the resource of backup texture
    EXPORT_API void _stdcall ReleaseResources(void** DevicePtr, void** ContextPtr)
    {
        ID3D11Device* pDevice = (ID3D11Device*)*DevicePtr;
        ID3D11DeviceContext* pContext = (ID3D11DeviceContext*)*ContextPtr;
        pDevice->Release();
    ...
    EXPORT_API void* _stdcall encD3D(void** ContextPtr, int width, int height, int gop, int qp)
        //initialize the encoder
    {
        ...
        try
        {
            ...
            // modify the group of pictures
            std::string str_gop = std::to_string(gop);
            oss << "-gop" << " " << str_gop << " ";

            // modify the rate control mode to constant QP and set the value of QP
            std::string str_qp = std::to_string(qp);
            oss << "-rc" << " " << "constqp" << " ";
            oss << "-constqp" << " " << str_qp << " ";
            ...
            {
                enc = new NvEncoderD3D11(pDevice.Get(), width, height, bForceNv12 ? NV_ENC_BUFFER_FORMAT_NV12 : NV_ENC_BUFFER_FORMAT_ARGB);
            ...
            //pass the pContext to a reference
            *ContextPtr = (void*)pContext.Get();
    ...
    // encode the frames
    EXPORT_API int _stdcall Encode(void* rTex, void* encPtr, void** ContextPtr, int s_width, int s_height, int width, int height)
    {
        //ComPtr<ID3D11Device> pDevice;
        //ComPtr<ID3D11DeviceContext> pContext;
        NvEncoderD3D11* enc = (NvEncoderD3D11*)encPtr;
        ...
        //copy a region of the resource
        D3D11_BOX sourceRegion;
        sourceRegion.left = s_width;
        sourceRegion.right = s_width + width;
        sourceRegion.top = s_height;
        sourceRegion.bottom = s_height + height;
        sourceRegion.front = 0;
        sourceRegion.back = 1;

        pContext->CopySubresourceRegion(pTexBgra, 0, 0, 0, 0, d3dTex, 0, &sourceRegion);
        pContext->Flush(); //TODO: try it tomorrow
        ...
        //pContext->Release(); // cannot release context here, or will lead to exception when destroy the encoder
    ...
    // get the encoded frames
    EXPORT_API char* _stdcall GetEncodedPacket(void* encPtr, int& pkg_size, bool enc_end )
    {
        NvEncoderD3D11* enc = (NvEncoderD3D11*)encPtr;
    ...
    EXPORT_API int _stdcall resetEnc(void* encPtr) {
        //nFrame = 0;
        NvEncoderD3D11* enc = (NvEncoderD3D11*)encPtr;
        enc->DestroyEncoder();
        //pDevice->Release();
