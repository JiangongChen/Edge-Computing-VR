...
std::streamsize ReadInputFrame(const NvEncInputFrame* encoderInputFrame, std::ifstream &fpBgra, char *pHostFrame, ID3D11DeviceContext *pContext, RGBToNV12ConverterD3D11 *pConverter,
                               ID3D11Texture2D *pTexSysMem, int nSize, int nHeight, int nWidth, bool bForceNv12, ID3D11Texture2D* d3dTex)
{
            ...
            std::ifstream &fpBgra, std::ofstream &fpOut, ID3D11Texture2D* d3dTex)
{
    ...
    while (true) 
    {
        ...
        std::streamsize nRead = ReadInputFrame(encoderInputFrame, fpBgra, reinterpret_cast<char*>(pHostFrame.get()), pContext, pConverter, pTexSysMem,
                                               nSize, nHeight, nWidth, bForceNv12, d3dTex);
...
extern "C"
{
    ...
    //int main()
    {
        ...
        try
        {
            ...
            /*
            ID3D11Texture2D* pTempFrame = (ID3D11Texture2D*)rTex;
            ID3D11Texture2D* d3dTex;
            ck(pDevice->CreateTexture2D(&desc, NULL, &d3dTex));*/
            D3D11_MAPPED_SUBRESOURCE map;
            int nSize = nWidth * nHeight * 4;
            /*std::unique_ptr<uint8_t[]> pTempFrame(new uint8_t[nSize]);
            int i = 0;
            while (i < 200) {
                std::streamsize nRead = fpBgra.read(reinterpret_cast<char*>(pTempFrame.Get()), nSize).gcount();
                i++;
            }*/
            /*
            ck(pContext->Map(d3dTex, D3D11CalcSubresource(0, 0, 1), D3D11_MAP_WRITE, 0, &map));
            for (int y = 0; y < nHeight; y++)
            {
                memcpy((uint8_t*)map.pData + y * map.RowPitch, reinterpret_cast<char*>(pTempFrame) + y * nWidth * 4, nWidth * 4);
            }
            pContext->Unmap(d3dTex, D3D11CalcSubresource(0, 0, 1));
            */
            //pContext->CopyResource(d3dTex, pTempFrame);
            /*ID3D11DeviceContext* ctx = NULL;
            pDevice->GetImmediateContext(&ctx);
            // Update texture data, and free the memory buffer
            ctx->UpdateSubresource(d3dTex, 0, NULL,pTempFrame , nWidth*4, 0);
            delete[](unsigned char*)pTempFrame;
            ctx->Release();*/
            ...
            else
            {
                Encode(pDevice.Get(), pContext.Get(), pConverter.get(), nWidth, nHeight, encodeCLIOptions, bForceNv12,
                    pTexSysMem.Get(), fpBgra, fpOut, d3dTex);
            }
