...
extern "C"
{
    EXPORT_API int _stdcall encD3D(void* rTex, int width, int height, int gop, int qp)
    ...
    // encode the frames
    EXPORT_API char* _stdcall Encode(void* rTex, int& pkg_size, bool enc_end, int s_width, int s_height)
    {
        ...
        if (!enc_end)
        {
            ...
            sourceRegion.right = s_width + nWidth;
            sourceRegion.top = s_height;
            sourceRegion.bottom = s_height + nHeight;
